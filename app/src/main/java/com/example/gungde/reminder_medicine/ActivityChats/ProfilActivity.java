               ...
               /* btn_declineFriend.setVisibility(View.INVISIBLE);
                ...
                if (mCurrentState.equals("req_received")) {
                    ...
                    final String kategori = pref.getString("kategori", "0");
                        ...
                        Call<GetKategori> call2 = api.getKategori(email);
                        call2.enqueue(new Callback<GetKategori>() {
                            @Override
                            public void onResponse(Call<GetKategori> call, Response<GetKategori> response) {
                                if (response.isSuccessful()) {
                                    final String kategorii = response.body().getData().get(0).getKategori();
                                    Map friendsMap = new HashMap();
                                    friendsMap.put("Friends/" + mFirebaseUser.getUid() + "/" + userid + "/date", currentDate);
                                    friendsMap.put("Friends/" + mFirebaseUser.getUid() + "/" + userid + "/kategori_pengguna", kategorii);

                                    friendsMap.put("Friends/" + userid + "/" + mFirebaseUser.getUid() + "/date", currentDate);
                                    friendsMap.put("Friends/" + userid + "/" + mFirebaseUser.getUid() + "/kategori_pengguna", kategori);
...
//                        Map friendsMap = new HashMap();
//                        friendsMap.put("Friends/" + mFirebaseUser.getUid() + "/" + userid + "/date", currentDate);
//                        friendsMap.put("Friends/" + mFirebaseUser.getUid() + "/" + userid + "/kategori_pengguna", kategori);
//
//                        friendsMap.put("Friends/" + userid + "/" + mFirebaseUser.getUid() + "/date", currentDate);
//                        friendsMap.put("Friends/" + userid + "/" + mFirebaseUser.getUid() + "/kategori_pengguna", );
//
//                        friendsMap.put("Friend_request/" + mFirebaseUser.getUid() + "/" + userid, null);
//                        friendsMap.put("Friend_request/" + userid + "/" + mFirebaseUser.getUid(), null);
//
