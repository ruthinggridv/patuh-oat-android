...
public class MainActivity extends AppCompatActivity {
    ...
    private Listobatpasien listobatpasien;
    private Artikel artikel;
    private ArtikelKeluargaFragment artikelKeluargaFragment;
    private Chats chats;
        ...
        public boolean onNavigationItemSelected(@NonNull MenuItem item) {
            switch (item.getItemId()) {
                case R.id.navigation_home:
                    if (pref.getString("kategori", "").equals("Keluarga")) {
                    setFragment(beranda);
                } else {
                    setFragment(listobatpasien);
                }
                    return true;
                case R.id.navigation_artikel:
                    if (pref.getString("kategori", "").equals("Pasien")) {
                        setFragment(artikel);
                    } else {
                        setFragment(artikelKeluargaFragment);
                    }
                    return true;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        beranda = new Beranda();
        listobatpasien = new Listobatpasien();
        artikel = new Artikel();
        chats = new Chats();
        settings = new Settings();
        artikelKeluargaFragment = new ArtikelKeluargaFragment();

        pref = getSharedPreferences("medical", Context.MODE_PRIVATE);
        if (pref.getString("kategori", "").equals("Keluarga")) {
            setFragment(beranda);
        } else {
            setFragment(listobatpasien);
